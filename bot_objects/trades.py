...
class Trades:
    ...
    def new_trade(self, trade_id: str, order_type: str, long_or_short: str, enter_or_exit: str, price: float = 0.0, stop_limit_price: float = 0.0) -> dict:
    ...
    def modify_side(self, side: Optional[str], order_leg_id: int = 0) -> None:
        ...
        if side:
            self.side['orderLegCollection'][order_leg_id]['instruction'] = side.upper()
        else:
            self.side['orderLegCollection'][order_leg_id]['instruction'] = self.order_instructions[self.enter_or_exit][self.side_opposite]

    def add_box_range(self, profit_size: float = 0.0, percentage: bool = False, stop_limit: bool = False) -> None:
    ...
    def add_stop_loss(self, stop_size: float, percentage: bool = False) -> None:
        if not self._triggered_added:
            self._convert_to_trigger()
    ...
    def add_stop_limt(self, stop_size: float, limit_size: float, stop_percentage: bool = False, limit_percentage: bool = False) -> bool:
        if not self._triggered_added:
            self._convert_to_trigger()
    ...
    def _generate_order_id(self) -> str:
        ...
        if self.order:
            ...
            order_id = f"{self.symbol}_{self.side}_{self.enter_or_exit}_{timestamp}"
            
        return order_id

    def add_leg(self, order_leg_id: int, symbol: int, quantity: int, asset_type: str, sub_asset_type: str = None) -> List[Dict]:
        # define the leg
        leg = {}
        leg['instrument']['symbol'] = symbol
        leg['instrument']['assetType'] = asset_type
